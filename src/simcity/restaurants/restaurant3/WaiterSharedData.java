...
public class WaiterSharedData extends Role implements Waiter{
	...
	public void msgSitAtTable(Customer c, int table) {
		AlertLog.getInstance().logInfo(AlertTag.RESIDENT_ROLE, this.getName(),
				"The host tells the waiter to seat the customer at a table");
	...
	public void msgReadyToOrder(Customer cust) {
		AlertLog.getInstance().logInfo(AlertTag.RESIDENT_ROLE, this.getName(),
				"customer tells waiter he/she is ready to order");
	...
	public void msgOrderIsReady(Order order) { //message from cook
		AlertLog.getInstance().logInfo(AlertTag.RESIDENT_ROLE, this.getName(),
				"cook tells the waiter that the food is ready for table " + order.tableNumber);
...
//		System.out.println("msgAtHost");
	...
	private void DoSeatCustomer(WaiterSharedData w, Customer customer, int table) {
		...
		//Same with "table"
		AlertLog.getInstance().logInfo(AlertTag.RESIDENT_ROLE, this.getName(),
				w.getName() + " is seating " + customer + " at " + table);
