...
import restaurant2.CookAgent.CookingState;
...
public class CashierAgent extends Role implements Cashier {
	...
	public void msgNeedCheck(Waiter w, double price, int table) {
		print("Received Request for check.");
	...
	public void msgHeresMyCheck(Customer c, Check ch, double debt) {
		print("Got check from " + c.getName());
		c.setCash(c.getCash() - ch.moneyOwed - debt);
		if(c.getCash() < 0) {
			print("You don't have enough money. You owe $" + Math.abs(c.getCash()));
	...
	public void msgHereIsDeliveryCheck(Check ch, Market m) {
		print("Got check for delivery from " + m.getName() + " for $" + ch.moneyOwed);
		if(restaurantCash < ch.moneyOwed) {
			print("We are short by $" + (ch.moneyOwed - Math.abs(restaurantCash)));
		}
		else {
			print("Paid check for $" + ch.moneyOwed);
	...
	public void makeCheck(int index) {
		...
		checkOrders.get(index).timer.schedule(new TimerTask() {
			Object cookie = 1;
			public void run() {
				print("Done making check for " + checkOrders.get(ind).waiter.getName() + " at table " + checkOrders.get(ind).table + ", cookie=" + cookie);
